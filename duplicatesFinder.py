...
class DuplicateFinderUI(FramelessWindow):
    ...
    def __initInputUI(self):
        ...
        self.pivotWidget.setStyleSheet(r"""#inputPivot{background-color: rgb(245, 245, 245);border-right: 1px solid rgba(0, 0, 0, 15)}""")
    ...
    def infoButtonClick(self):
        if self.srcInfoBtn.isPressed:
            openFolder(self.srcInfoBtn.text())
        elif self.tarInfoBtn.isPressed:
            openFolder(self.tarInfoBtn.text())
        else:
            pass
...
class ImageFrame(CommonFrame):
    ...
    def mousePressEvent(self, event):
        ...
        if currentButton == Qt.RightButton:
            ...
            openAction = Action(FluentIcon.FOLDER, self.tr('打开目录'), triggered=self.openFolder)
    ...
    def openFolder(self):
        if self.imagePath is not None and self.imagePath.exists():
            openFolder(self.imagePath)
            if self.flyoutMenu is not None:
                self.flyoutMenu.close()

    def showImage(self):
        if self.imagePath is not None and self.imagePath.exists():
            startfile(self.imagePath)
            if self.flyoutMenu is not None:
                self.flyoutMenu.close()
...
def openFolder(path: str | Path):
    """
    windows 打开文件（文件夹）所在文件夹方式, 文件被选中
    """
    path = path if isinstance(path, Path) else Path(path)
    if not Path(path).exists():
        return
    process = QProcess()
    cmd = f'''explorer.exe /select, "{str(path).replace("/", "\\")}"'''
